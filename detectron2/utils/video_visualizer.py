...
class _DetectedInstance:
    ...
    __slots__ = ["label", "index", "bbox", "mask_rle", "color", "ttl"]

    def __init__(self, label, index, bbox, mask_rle, color, ttl):
        self.label = label
        self.index = index
        self.bbox = bbox
    ...
    def draw_instance_predictions(self, frame, predictions):
        ...
        if not predictions.has("COLOR"):
            if predictions.has("ID"):
                ...
                detected = [
                    _DetectedInstance(classes[i], i, boxes[i], mask_rle=None, color=colors[i], ttl=8)
